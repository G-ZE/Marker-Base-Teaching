...
class DataManagement
{
	...
	public:
		 ...
		 * */
		bool circleEstimation (cv::Mat& input_image, cv::Mat& input_image2, int blur_param_, int hsv_target_, int hsv_threshold_ , int contour_area_min_, int contour_area_max_,  double contour_ratio_min_, double contour_ratio_max_, double contour_length_ratio_min_, double contour_length_ratio_max_){
			
			int marker_count_ = 0;
			annoted_frame_ = input_image.clone();
			annoted_ref_frame = input_image2.clone();
			...
			cv::Mat image2_blurred;
			cv::medianBlur(input_image2, image2_blurred, blur_param_);
			...
			cv::inRange(image_hsv,cv::Scalar(hsv_target_ - hsv_threshold_,20,0), cv::Scalar(hsv_target_ + hsv_threshold_,255,255),image_hsv_filtered);
			std::cout<<" HSV2 filtering " <<std::endl;
			cv::Mat image2_hsv;
			cv::Mat image2_hsv_filtered;
			cv::cvtColor(image2_blurred, image2_hsv, CV_BGR2HSV);
			cv::inRange(image2_hsv,cv::Scalar(hsv_target_ - hsv_threshold_,20,0), cv::Scalar(hsv_target_ + hsv_threshold_,255,255),image2_hsv_filtered);
			...
			cv::Mat erosion_dst, dilation_dst, erosion_dst2, dilation_dst2;
			...
			cv::erode( image2_hsv_filtered, erosion_dst2, e_element );
			cv::dilate( erosion_dst2, dilation_dst2, d_element );
			...
			cv::findContours(dilation_dst2, contours2, hierarchy2, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0) );
			
			//#5 Filter Unnecessary Contours
			cv::Mat image_contour_filtered =  cv::Mat::zeros( input_image.size(), CV_8U);
			std::vector<cv::Point2f> ref_points_list;
			
			//ref points for comparsion
			for (int i = 0 ; i < contours2.size() ; i++){
					double contour_area = cv::contourArea(contours2[i]);
					double contour_length = cv::arcLength(cv::Mat(contours2[i]),true);
					if((contour_area < contour_area_max_) && (contour_area > contour_area_min_)){
                        //std::cout<<"contour2 in area range "<<std::endl;
                        cv::drawContours(annoted_ref_frame, contours2, i, cv::Scalar(255, 0, 0), 1, 8);
                        //all contours in area range
                        //cv::drawContours(annoted_frame_, contours, i, cv::Scalar(255, 0, 0), 1, 8);
                   
                        
						    //#6 Check for Child Contours
						    for (int j = i; j < hierarchy2.size() ; j++){
							    if((hierarchy2[j][3]==i) && (hierarchy2[j][2]==-1)){	
								    double child_area_ = cv::contourArea(contours2[j]);
								    double contour_ratio_ = contour_area / child_area_;
								    double contour_length_child_ = cv::arcLength(cv::Mat(contours2[j]),true);
								    ...
								    if((contour_ratio_max_ >= contour_ratio_) && (contour_ratio_min_ <= contour_ratio_) && (contour_length_ratio_max_>= contour_length_ratio_) && (contour_length_ratio_min_<=contour_length_ratio_)){
									    std::vector<std::vector<cv::Point> > con_out = std::vector<std::vector<cv::Point> >(1, contours2[i]);
									    std::vector<std::vector<cv::Point> > con_in = std::vector<std::vector<cv::Point> >(1, contours2[j]);
									   ...
									   //std::cout<< "deta_x2 "<< deta_x<<" deta_y2 "<<deta_y<<std::endl; 
									    //#7 checking for center differents
									    if ((deta_x <= 1) && (deta_x >= -1) && (deta_y <= 1) && (deta_y >= -1)){
											cv::drawContours(annoted_ref_frame, contours2, i, cv::Scalar(0, 255, 0), 1, 8);
											cv::drawContours(annoted_ref_frame, contours2, j, cv::Scalar(0, 0, 255), 1, 8);
											std::cout<<"marker1 confirmed "<<endl;
											ref_points_list.push_back(p_in);
							    ...
							    if((hierarchy[j][3]==i) && (hierarchy[j][2]==-1)){	
								    ...
								    if((contour_ratio_max_ >= contour_ratio_) && (contour_ratio_min_ <= contour_ratio_) && (contour_length_ratio_max_>= contour_length_ratio_) && (contour_length_ratio_min_<=contour_length_ratio_)){
									    ...
									    if ((deta_x <= 1) && (deta_x >= -1) && (deta_y <= 1) && (deta_y >= -1)){
											...
											//checking with ref list
											
											//compare and confirm
											for ( int k =0; k< ref_points_list.size() ; k++){
												cv::Point2f p_ref = ref_points_list[k];
												double diff_x = p_in.x - p_ref.x;
												double diff_y = p_in.y - p_ref.y;
...
bool DataManagement::detectMarkers(int blur_param_, int hsv_target_, int hsv_threshold_ , int contour_area_min_, int contour_area_max , double contour_ratio_min_, double contour_ratio_max_, double contour_length_ratio_min_, double contour_length_ratio_max_, bool aruco_detection_){
	if(image_ready){
		...
		if(aruco_detection_){
			...
			marker_found = circleEstimation(frame, refframe, blur_param_, hsv_target_, hsv_threshold_ , contour_area_min_, contour_area_max, contour_ratio_min_, contour_ratio_max_, contour_length_ratio_min_, contour_length_ratio_max_);
